package cl.scripts.genetaredummytest;
...
final class DummyTest {
	...
	private void generarClass(final StringBuffer buff) {
		...
		buff.append("\tpublic static final String NOMBRE_DIRECTORIO = \"" + getDIRECTORIOS() + "\";\n");
		...
		if (getINCLUIR_PRIV_MET()) {
			toDO(buff, "Se crea un userDTO, especificar un mapa con parametros si es necesario", 2);
			...
			buff.append("\t\tllamadaMetodosPrivados(objeto, metodos);\n");
		}
	...
	private void generarMetodoLlamadasPrivadas(final StringBuffer buff) {
		...
		buff.append("\t\t\tfinal Map<String, Object> metodos) {\n");
